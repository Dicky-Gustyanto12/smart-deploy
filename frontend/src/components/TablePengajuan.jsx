...
export default function TableStat() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch("http://backend-alsintan.test/api/poktan").then((res) =>
        res.json()
      ),
      fetch("http://backend-alsintan.test/api/pengajuan").then((res) =>
        res.json()
      ),
    ])
  ...
  const handleSave = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (editMode) {
          fetch(`http://backend-alsintan.test/api/pengajuan/${id_pengajuan}`, {
            method: "PUT",
          ...
          fetch("http://backend-alsintan.test/api/pengajuan", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          })

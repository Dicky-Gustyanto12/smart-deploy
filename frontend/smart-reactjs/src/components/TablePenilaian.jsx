...
export default function TablePenilaian() {
  ...
  useEffect(() => {
    fetchData();
    fetch("https://api.alsindata.id/api/kriteria")
      .then((res) => res.json())
      .then(setKriteriaAll);
    fetch("https://api.alsindata.id/api/parameter")
      .then((res) => res.json())
  ...
  const fetchData = () => {
    ...
    Promise.all([
      ...
      .then(([poktanResult, penilaianResult]) => {
        ...
        setPoktanMap(
          Object.fromEntries(poktanResult.map((p) => [p.id_poktan, p.nama_poktan]))
        );
  ...
  const handleSave = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const url = editId
          ? `https://api.alsindata.id/api/penilaian/${editId}`
          : "https://api.alsindata.id/api/penilaian";
        fetch(url, {
          method,
  ...
  const handleDelete = (id_penilaian) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setLoading(true);
        fetch(`https://api.alsindata.id/api/penilaian/${id_penilaian}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then(() => {

...
export default function TableSkor() {
  ...
  return (
    <div className="p-4 w-full max-w-6xl mx-auto">
      <h2 className="text-lg font-bold mb-4 text-gray-800">Nilai Akhir Metode SMART</h2>
          ...
          {loading ? (
            ...
            <tbody>
              {resultMatrix.map((row, idx) => (
                <tr key={row.kode} className={idx % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                  <td className="py-2 px-4 font-semibold">{row.kode}</td>
                  <td className="py-2 px-4">{row.nama}</td>
                  {row.skor.map((val, i) => (
                    <td key={i} className="py-2 px-4">{val.toFixed(2)}</td>
                  ))}
                  <td className="py-2 px-4 font-bold">{row.total.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          )}

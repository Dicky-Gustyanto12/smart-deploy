...
export default function TableParameter() {
  ...
  const fetchKriteria = async () => {
    try {
      const res = await fetch("https://api.alsindata.id/api/kriteria");
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://api.alsindata.id/api/parameter");
      let result = await res.json();
  ...
  const handleSave = async () => {
    ...
    try {
      let url = "https://api.alsindata.id/api/parameter";
      let method = "POST";
      if (editMode) {
  ...
  const handleDelete = async (id_parameter) => {
    ...
    try {
      await fetch(`https://api.alsindata.id/api/parameter/${id_parameter}`, {
        method: "DELETE",
      });
      await fetchData();

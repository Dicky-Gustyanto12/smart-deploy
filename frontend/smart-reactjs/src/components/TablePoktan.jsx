...
export default function TablePoktan() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let res, updated;
      if (editMode) {
        res = await fetch(`https://api.alsindata.id/api/poktan/${id_poktan}`, {
          method: "PUT",
        ...
        res = await fetch("https://api.alsindata.id/api/poktan", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            nama_poktan,
  ...
  const handleDelete = (id_poktan) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setLoading(true);
        fetch(`https://api.alsindata.id/api/poktan/${id_poktan}`, {
          method: "DELETE",
        })
          .then((res) => {

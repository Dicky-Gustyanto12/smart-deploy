...
export default function TablePengajuanWithModal() {
  ...
  const openEditStatusModal = (row) => {
    setEditStatusModal({ open: true, row, status: row.status, dropdownOpen: false });
  };
  const closeEditStatusModal = () =>
    setEditStatusModal({ open: false, row: null, status: "", dropdownOpen: false });
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(
            `https://api.alsindata.id/api/pengajuan/${row.id_pengajuan}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id_pengajuan) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(
            `https://api.alsindata.id/api/pengajuan/${id_pengajuan}`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            <tbody>
              {data.map((item, idx) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">{formatTanggalOnly(item.updated_at)}</td>
                  <td className="py-3 px-4">
                    <button

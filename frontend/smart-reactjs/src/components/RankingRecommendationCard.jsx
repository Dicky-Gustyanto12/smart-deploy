...
const RankingRecommendationCard = ({ className }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
    ...
    ]).then(([penilaianAll, kriteriaAll, poktanAll]) => {
      ...
      const cmin = kriteriaKeys.map((_, i) =>
        matrixRows.length === 0 ? 0 : Math.min(...matrixRows.map((row) => row.nilai[i]))
      );
      const cmax = kriteriaKeys.map((_, i) =>
        matrixRows.length === 0 ? 0 : Math.max(...matrixRows.map((row) => row.nilai[i]))
      );
      ...
      const matrixNorm = matrixRows.map((row) => ({
        kode: row.kode,
        nama: row.nama,
        norm: row.nilai.map((v, i) => (cmaxmin[i] === 0 ? 0 : (v - cmin[i]) / cmaxmin[i])),
      }));
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-2">
        <span className="font-bold text-2xl text-[#2c342f]">Ranking Kelompok Tani</span>
      </div>
        ...
        {loading
          ...
          : resultMatrix.map((item, index) => (
              ...
              >
                ...
                <div>
                  <div className="font-bold text-white text-shadow-xl">{item.nama}</div>
                  <div className="text-xs text-gray-100">{item.kode}</div>
                  {index === 0 && (
                    <div className="text-xs text-white font-semibold">Sangat Direkomendasikan</div>
                  )}
                </div>
              </div>
            ))}
